...
def train_REINFORCE_baseline_entropy(env, policy_net, policy_optimizer, value_net, value_optimizer, gamma=0.99, entropy_coef=0.002):
    """
    ...
    # Update policy network weights
    L_theta = torch.stack([-log_prob * advantage - entropy_coef * ent for log_prob, advantage, ent in zip(log_probs, advantages, entropy)]).sum() / T
...
for i in tqdm.tqdm(range(num_seeds)):
    ...
    for m in range(num_episodes):
        ...
        train_REINFORCE_baseline_entropy(env, policy_net, policy_net_optimizer, value_net, value_net_optimizer, gamma=gamma)
        if m % 100 == 0:
...
np.save(os.path.join(directory, "returns_reinforcement_baseline_entropy_0.002.npy"), returns)
